package com.rom4ek.curvenavview
...
class CurveNavView : NavigationView {

    private var settings: CurveViewSettings? = null
    ...
    fun init(context: Context, attrs: AttributeSet?) {
        settings = CurveViewSettings(context, attrs)
        ...
        THRESHOLD = Math.round(CurveViewSettings.dpToPx(getContext(), 15)) //some threshold for child views while remeasuring them
    }
    ...
    override fun measureChild(child: View, parentWidthMeasureSpec: Int, parentHeightMeasureSpec: Int) {
        if (child is NavigationMenuView) {
            child.measure(MeasureSpec.makeMeasureSpec(measuredWidth,
